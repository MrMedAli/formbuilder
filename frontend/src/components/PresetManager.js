...
const PresetManager = () => {
  ...
  const fetchResponses = async () => {
    try {
      ...
      const response = await axios.get("http://localhost:8000/api/responses/", { headers });
  ...
  const handleEditSave = async () => {
    try {
      ...
      await axios.put(`http://localhost:8000/api/responses/${editResponse.id}/`, responseData, { headers });
