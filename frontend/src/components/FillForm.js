...
const FillForm = () => {
  ...
  const fetchForm = async (formId) => {
    try {
      const headers = authService.getAuthHeader();
      const response = await axios.get(`${apiUrl}/api/forms/${formId}/`, { headers });
  ...
  const fetchSavedResponses = async (formId) => {
    try {
      const headers = authService.getAuthHeader();
      const response = await axios.get(`${apiUrl}/api/responses/?form=${formId}`, { headers });
  ...
  const handleSave = async () => {
    try {
      ...
      await axios.post(
        `${apiUrl}/api/forms/${selectedFormId}/submit/`,
